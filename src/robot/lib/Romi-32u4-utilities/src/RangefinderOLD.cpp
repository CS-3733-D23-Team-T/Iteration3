...
void Rangefinder::init(void)
{
  ...
  if(echoPin == 17){
    ...
    {
        Serial.println("Attaching rangefinder ISR");
        attachInterrupt(digitalPinToInterrupt(echoPin), ::ISR_RangefinderSide, CHANGE);
    }
    else if(digitalPinToPCInterrupt(echoPin) != NOT_AN_INTERRUPT)
    {
        Serial.println("Attaching rangefinder PC_ISR");
        attachPCInt(digitalPinToPCInterrupt(echoPin), ::ISR_RangefinderSide);
    }
  }
  else
  {
...
void Rangefinder::ISR_echo_side(void)
{
    if(digitalRead(echoPin))  //transitioned to HIGH
    {
        pulseStartS = micros();
...
void ISR_Rangefinder(void)
{
    ...
    frontRangefinder.ISR_echo();
}
void ISR_RangefinderSide(void){
