 ...
 * */
RangefinderSide::RangefinderSide(uint8_t echo, uint8_t trig) 
{
    echoPin = echo;
    trigPin = trig;
}
...
void RangefinderSide::init(void)
{
  ...
  if(digitalPinToInterrupt(echoPin) != NOT_AN_INTERRUPT)
  {
    ...
    attachInterrupt(digitalPinToInterrupt(echoPin), ::ISR_Rangefinder_Side, CHANGE);
  }
  else if(digitalPinToPCInterrupt(echoPin) != NOT_AN_INTERRUPT)
  {
    Serial.println("Attaching rangefinder PC_ISR");
    attachPCInt(digitalPinToPCInterrupt(echoPin), ::ISR_Rangefinder_Side);
  }
  else
  {
 ...
 */
uint8_t RangefinderSide::checkPingTimer(void)
{
...
uint16_t RangefinderSide::checkEcho(void)
{
...
float RangefinderSide::getDistance(void)
{
 ...
 * */
void RangefinderSide::ISR_echo_side(void)
{
 ...
 * */
void ISR_Rangefinder_Side(void)
{
    sideRangefinder.ISR_echo_side();
}